...
def framedSend(sock, payload, debug=0):
     ...
     if debug: print("framed message length= %d bytes.  type=%s, last 3 bytes are <%s>" % (len(msg), type(msg), msg[-3:]))
     while len(msg):
         nsent = sock.send(msg)
         msg = msg[nsent:]
    ...
    while True:
         ...
         if state == "getPayload":
             if len(rbuf) >= msgLength:
                  if debug: print("msgLength=%d, len(rbuf=%d" % (msgLength, len(rbuf)))
                  payload = rbuf[0:msgLength]
                  rbuf = rbuf[msgLength:]
                  if debug and len(rbuf): print("trimmed rbuf=<%s>" % rbuf)
                  return payload
         r = sock.recv(100)
         rbuf += r
         if len(r) == 0:
